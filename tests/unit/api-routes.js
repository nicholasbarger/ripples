...
describe('api routes', function() {
  it('should allow returning a list of all ripples for the logged in user') {
  ...
  it('should allow returning a specific ripple by id') {
    // todo
    expect(ripple).toBe(stubbedRipple);
  };
  
  it('should allow creating a new ripple by specifying the unique _id') {
    // todo
    expect(ripple).toBe(stubbedRipple);
  };
  
  it('should allow creating a new ripple and auto-filling an _id if not specified') {
    // todo
    var _id = null;
    expect(ripple).toBe(stubbedRipple);
  };
  
  it('should allow updating an existing ripple') {
    // todo
    expect(ripple).toBe(stubbedRipple);
  };
  
  it('should allow executing a ripple by specified _id') {
    // todo
    expect(api.execute).toHaveBeenCalled();
  };
  
  it('should throw a 404 error if a matching route is not found') {
    // todo
    expect(false);
  };
  
  it('should throw an error if updating a ripple without the required fields') {
    var name = null;
  ...
  it('should throw an error if the sample input is not valid json') {
  ...
  it('should throw an error if the sample output is not valid json') {
  ...
  it('should throw an error if the ripple specifies code that is invalid') {
